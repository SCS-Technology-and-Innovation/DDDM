...
\begin{frame}{\textit{Behavior} for the \textbf{unary} operator \texttt{\bf A} ($\forall$)}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \onslide<9->{\node at (0, -1) {$\forall \varphi$};}

      \onslide<10->{\node [draw=none, fill=green!30, minimum width=95mm, minimum height=55mm] at (5.7, 1) {};}
      
      \onslide<10->{ \node at (6, -1) {on {\bf all} possible paths, $\varphi = \top$};}
    ...
    \only<10-12>{$\forall \varphi$ states that $\varphi$ is true along
      {\bf every} possible path.}

    \only<13-15>{This operator is called \textit{(for) all}.}
