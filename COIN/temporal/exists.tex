...
\begin{frame}{\textit{Behavior} for the \textbf{unary} operator \texttt{\bf E} ($\exists$)}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \onslide<9->{\node at (0, -1) {$\exists \varphi$};}

      \onslide<10->{\node [draw=none, fill=orange!30, minimum width=95mm, minimum height=55mm] at (5.7, 1) {};}
      
      \onslide<10->{ \node at (6, -1) {on {\bf at least one} possible path, $\varphi = \top$};}
    ...
    \only<10-12>{$\exists \varphi$ states that $\varphi$ is true along
      {\bf at least one} possible path.}

    \only<13-15>{This operator is called \textit{exists}.}
