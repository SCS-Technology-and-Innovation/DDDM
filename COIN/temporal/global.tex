...
\begin{frame}{\textit{Behavior} for the \textbf{unary} operator \texttt{\bf G} ($\square$)}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \onslide<9->{\node at (0, -1) {$\square \varphi$};}

      \onslide<10->{\node [draw=none, fill=purple!30, minimum width=95mm, minimum height=55mm] at (5.7, 1) {};}
      
      \onslide<10->{ \node at (6, -1) {{\bf everywhere} on {\em all} paths, $\varphi = \top$};}
    ...
    \only<10-12>{$\square \varphi$ states that $\varphi$ is true at
      {\bf every} future state\footnote{regardless of the path
      taken}.}

    \only<13-15>{This operator is called \textit{globally}.}
